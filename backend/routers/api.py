...
router = APIRouter(prefix="/api", tags=["Elo Engine"])
...
def get_next_match(
    ...
    if not match:
        raise HTTPException(
            status_code=404, detail="No more matches available to vote on"
        )
    return match
...
async def upload_image(
    ...
    if not supabase:
        raise HTTPException(
            status_code=500, detail="Supabase client not configured in .env"
        )
...
def update_admin_photo(
    ...
    updated = crud.update_photo(
        db, photo_id, elo_rating=update.elo_rating, filename=update.filename
    )
    if not updated:
