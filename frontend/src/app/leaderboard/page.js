    ...
    if (typeof window !== "undefined") {
      ...
      if (params.get("limit_reached") === "true") {
        ...
        if (nextReset) {
          ...
          setResetTime(
            tomorrowUTC.toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
              timeZoneName: "short",
            }),
              ...
              <div key={photo.id} className={`rank-item rank-${index + 1}`}>
                ...
                <div className="rank-info">
                  <div className="rank-score">
                    Elo: {Math.round(photo.elo_rating)}
                  </div>
                  <div className="rank-matches">
                    {photo.matches_played} matches played
                  </div>
                </div>
              </div>
            ))
        ...
        {limitReached && (
          ...
          >
            ...
            <p style={{ fontSize: "1.1rem" }}>
              You will be able to vote again at{" "}
              <strong style={{ color: "white" }}>{resetTime}</strong>.
            </p>
          </div>
        )}
      </div>
